...
void AParser::SetString(const string &sec, const string &key,
                        const string &value) {
...
void AParser::SetFloat(const string &sec, const string &key,
                       const float value) {
  ostringstream ostr;
  ostr << value;
...
void AParser::clearAllSection() { fSections.clear(); }
...
void AParser::ASectionVector::Store(ASection *pSec) {
  if (NULL != pSec) {
    this->push_back(*pSec);
